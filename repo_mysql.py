...
class repo_mysql:
    ...
    def _getRenrenId(self, col, renrenId):
        ...
        res = set()
        self.cur.execute(
            "SELECT renrenId{} FROM {} where renrenId{}={}".format(target, self.table_name[pageStyle], where, renrenId))
        for item in self.cur.fetchall():
            res.add(item[0])
        return res
    ...
    def sql_history(self, rid, pageStyle, run_info, n_record):
        return "insert into {} (rid,page_style,run_info,n_record) values('{}','{}','{}',{})".format(
            self.table_name['history'], rid, pageStyle, run_info, n_record)
    ...
    def _sql_friendList(self, record, rid):
        ...
        sql_relation = "insert into {} (renrenId1,renrenId2) values ('{}','{}')".format(self.table_name[pageStyle], rid,
                                                                                        val_relation)
