...
urls = {
    ...
    'albumList': "http://photo.renren.com/photo/{id}/album/relatives"
}
itemReg = {
...
def format_time(runtime, req_time=None):
    """
    convert runtime into string
    statistics about all request times if available
    :param runtime: running time
    :param req_time: iterable list of request times
    """
    if req_time is None:
        return '%.2f' % runtime
...
class browser:
    ...
    def status(self, renrenId='285060168', uppage=100):
        """
        status(renrenId:str) --> (record:dict(), timecost:str),
        return (None, error_info) if error
        """
        return self.process('status', renrenId, uppage)

    def profile(self, renrenId='234234', no_use=None):
        """
        profile(renrenId:str) --> (record:dict(), timecost:str)
        return (None, error_info) if error
        """
